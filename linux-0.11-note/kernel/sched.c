 ...
 */
/*
 * 'sched.c'是主要的内核文件。其中包括有关调度的基本函数(sleep_on、wakeup、schedule
 * 等)以及 一些简单的系统调用函数（比如 getpid()，仅从当前任务中获取一个字段）。
 */
#include <asm/io.h>
...
//  取信号 nr 在信号位图中对应位的二进制数值。信号编号 1-32    比如信号 5
//  的位图数值 = 1<<(5-1) = 16 = 00010000b
#define _S(nr) (1 << ((nr)-1))
...
/**
 * @brief
 * 显示任务号nr的进程号、进程状态和内核堆栈空闲字节数（内核堆栈大小为4k，用task_union存储）
 *
 * @param nr 任务号
 * @param p 任务结构体
 */
void show_task(int nr, struct task_struct* p)
{
    ...
    i = 0;
    while (i < j &&
           !((char*)(p + 1))[i])  // 检测指定任务数据结构以后等于 0 的字节数
        i++;
...
void schedule(void)
{
    ...
    for (p = &LAST_TASK; p > &FIRST_TASK; --p)
        if (*p)
        {  // 如果任务的 alarm 时间已经过期(alarm<jiffies),在信号位图中置
           // SIGALRM 信号，然后清 alarm
            ...
            if (((*p)->signal &
                 ~(_BLOCKABLE & (*p)->blocked)) &&  // 则置任务为就绪状态
    ...
    else
    {
        for (p = timer_list; p < timer_list + TIME_REQUESTS; p++)
            if (!p->fn)  // 从定时器数组中，找一个空闲项
                break;
        if (p >=
            timer_list + TIME_REQUESTS)  // 如果已经用完了定时器数组，则系统崩溃
...
void do_timer(long cpl)  // current priority level
{
    ...
    // 如果有用户的定时器存在，则将链表第 1 个定时器的值减 1。如果已等于
    // 0，则调用相应的处理 程序，并将该处理程序指针置为空。然后去掉该项定时器
    if (next_timer)
    {
        next_timer->jiffies--;
...
void sched_init(void)
{
    ...
    set_tss_desc(gdt + FIRST_TSS_ENTRY,
                 &(init_task.task.tss));  // 设置初始任务, 非常重要：初始化进程0
    ...
    outb(LATCH >> 8, 0x40);
    /* MSB */  //  这四行代码就开启了这个定时器，之后这个定时器变会持续的、以一定频率的向
               //  CPU 发出中断信号， 中断处理程序为 timer_interrupt
    set_intr_gate(0x20, &timer_interrupt);
    outb(inb_p(0x21) & ~0x01, 0x21);
    set_system_gate(
        0x80,
        &system_call);  // 所有用户态程序想要调用内核提供的方法，都需要基于这个系统调用来进行
}
