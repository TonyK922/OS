...
int copy_mem(int nr, struct task_struct* p)
{
    ...
    new_data_base = new_code_base =
        nr * 0x4000000;  // 新基址=任务号*64Mb(任务大小)
...
/*
 * OK，下面是主要的 fork
 * 子程序。它复制系统进程信息(task[n])并且设置必要的寄存器。
 * 它还整个地复制数据段。
 */
// 复制进程。
// 复制task_struct（包含tss和ldt等），复制寄存器，复制内存；再进行一些个性化设置（如各种id，状态等）
                 ...
                 long ss)
{
    ...
    task[nr] = p;
    *p       = *current;
    /* NOTE! this doesn't copy the supervisor stack */  // 完全复制之前的进程的task_struct
    p->state =
        TASK_UNINTERRUPTIBLE;  // 很重要，此时进程还未复制完成，不能被进程调度打断
    p->pid     = last_pid;
    ...
    /* do this last, just in case */  // 复制完成之后置为可运行状态
    return last_pid;
}
